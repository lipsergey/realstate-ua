...
<?php
//%%
include("/home/farmait/www/config.php");
...
if ($MinutSQL != "") {
	...
	echo "SELECT Task_ID, Task_RunFile
	FROM ".$ppt."crontb_tasks
	WHERE Task_Active='1'
	AND UNIX_TIMESTAMP(DATE_FORMAT(Task_LastStart, '%Y-%m-%d %H:%i:00')) < UNIX_TIMESTAMP(DATE_FORMAT(NOW(), '%Y-%m-%d %H:%i:00'))
	AND (Task_Year='*' OR Task_Year='".date("Y")."')
	...
	while($TaskSpis = mysqli_fetch_assoc($crontabspis)) {
		if (file_exists($SPUrl . $TaskSpis["Task_RunFile"])) {
			echo "Run: ".$SPUrl . $TaskSpis["Task_RunFile"]."<Br><br>";
			include($SPUrl . $TaskSpis["Task_RunFile"]);
			//TMP LOCK $r=mysqli_query($hlnk, "UPDATE ".$ppt."crontb_tasks SET Task_LastStart=NOW() WHERE Task_ID='".$TaskSpis["Task_ID"]."';") or die("Update Cron :(");
		}
		else {
			//TMP LOCK $r=mysqli_query($hlnk, "UPDATE ".$ppt."crontb_tasks SET Task_Active='0' WHERE Task_ID='".$TaskSpis["Task_ID"]."';") or die("Deactiv :(");
		}
	}
}
die();
