...
продолжил webi.ru (adm@webi.ru),
модификация для совместимости с системой - Sergey Lipsits (sergey@lipsits.ru)

Пример использования:
$m= new sendletter('windows-1251');  // можно сразу указать кодировку, можно ничего не указывать ($m= new Mail;)
...
class sendletter {
	...
	var $acc = array();
	var $abcc = array();
	var $aattach = array(); // прикрепляемые файлы
	var $xheaders = array(); //массив заголовков
	var $priorities = array( '1 (Highest)', '2 (High)', '3 (Normal)', '4 (Low)', '5 (Lowest)' ); //приоритеты
	var  $charset = "windows-1251"; //кодировка по умолчанию
	var  $ctencoding = "8bit";
	var  $receipt = 0;
	...
	*/
	...
	function sendletter($charset="")
	{
		...
		$this->ReplyTo(SRVMAILPODPIS.";".SRVMAILADDR);
	}
	...
	*/
	...
	function BuildMail() {
		...
		$this->xheaders['Return-Path'] = SRVMAILADDR; // заголовок для возврата
		$this->xheaders['X-PHP-Script'] = "sendletter";
		$this->xheaders['X-PHP-Originating-Script'] = "sendletter";
		...
		$this->xheaders["X-Mailer"] = "Pinwin mail agent";
	...
	*/
	function Send()
	{
		...
		if(!$this->smtp_on)
		{
			...
			$this->fullBody = str_replace("\r\n","\n",$this->fullBody);
			$this->fullBody = trim($this->fullBody);
			//$this->headers = str_replace("\r\n","\n",$this->headers);
			
			$res = @mail( $this->strTo, $this->xheaders['Subject'], $this->fullBody, $this->headers, "-f".SRVMAILADDR );
