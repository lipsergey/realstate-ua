...
class sendletter
{
     ...
     * @access private  
     */
	private $charset = "UTF-8"; //was "windows-1251"; 
     ...
     * @var string 
     */
	private $ctencoding = "8bit"; //was "8bit";
     ...
     * @var boolean 
     */
    private $checkAddress = false;//true;
    ...
    public function __construct($charset = "", $ctencoding = '')
    {
		...
		/*
		$this->smtp['serv'] = 'ssl0.ovh.net';
		$this->smtp['login'] = $popMailParams["login"];
		$this->smtp['pass'] = $popMailParams["passw"];
		$this->smtp['port'] = 465;
		...
		*/
		...
		if (!defined("SRVMAILADDR")) {
			mail("lipsits@gmail.com", "Sendletter class on farmait error", "SendLetter class on farmait  error: no constant SRVMAILADDR. Starting script: ".basename(__FILE__));
			die();
		}
 		$this->From(SRVMAILPODPIS.";".SRVMAILADDR);
	}

	//Специальная функция создания метки инициатора рассылки: папка и имя файла запустившего рассылку.
	//А также ID шаблона письма
	function AddToBodyStart($text, $text_html = "") {
		$t = dirname($_SERVER["SCRIPT_FILENAME"]);
		$Al = explode(DIRECTORY_SEPARATOR, $t);
		$ATxt = "run: [".array_pop($Al) . DIRECTORY_SEPARATOR . basename($_SERVER["SCRIPT_FILENAME"]) . "]";
		if (is_numeric($this->TemplateID) && $this->TemplateID > 0) {
			$ATxt .= "; templ: [".$this->TemplateID."]";
			$this->TemplateID = 0;
		}
		if ($text_html == "html") {
			return "<br><br><br><font size=-2>Letter key: <<".str_replace('==','',base64_encode($ATxt)).">></font>";
		}
		else{ 
			return "\n\n\nLetter key: <<".str_replace('==','',base64_encode($ATxt)).">>";
		}
	}
    ...
    {
		//$text .= $this->AddToBodyStart($text, $text_html);
     ...
     * @return boolean
     */
    public function Attach($filename, $new_name_filename = "", $filetype = "", $disposition = "attachment", $resource = 'webi')
    {
        if (!strlen($resource))
            $resource = 'webi';
...
// если установлено подтверждение о доставке, берем адрес куда слать уведомление из адрес для ответа, либо из From
	...
	
    {
        ...
        // Если в режиме разработчика, то не отправлять писем, складировать в директорию в фс
        if (defined('DEV_MODE') && DEV_MODE) {

            if (!file_exists(DEV_EMAILS_PATH) || !is_writable(DEV_EMAILS_PATH)) {
                return false;
            }
            ...
            foreach ($this->sendto as $key => $value) {
                ...
                $newFilename = DEV_EMAILS_PATH . '/' . $strTo . '_' . date('Y-m-d_H-i') . '.txt';
                if (!touch($newFilename)) {
                    continue;
                }

                $message = $this->headers[$key]['Subject'] . PHP_EOL
                    . $this->body[$key];
                file_put_contents($newFilename, $message);
            }
        ...
        if (!$this->smtp['on'])
        {
            ...
            foreach ($this->sendto as $key => $value)
            {
                ...
                if (isset($this->body[$key])) {$body_resource = $key;}
...
// разбиваем (FROM - от кого) на юзера и домен. юзер понадобится в приветсвии с сервом
            ...
            foreach ($this->smtpsendto as $key_res => $value_res)
            {
                ...
                if (isset($this->body[$key_res])) {$body_resource = $key_res;}
	...
	public static function initWithConfigCharset($ctencoding = '')
    {
		return new self();
	/* LOCK 2015-02-22  NOT USE IT ANYMORE

        if(!defined('UNICODE_NOT_SUPPORTED') || UNICODE_NOT_SUPPORTED === false)
            return new self('UTF-8', $ctencoding);
        
        return new self('windows-1251', $ctencoding);
	*/
    }
